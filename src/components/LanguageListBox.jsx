...
let findDefaultLang = (searchparam) => {
  let lang = languages.find((language) => language.iso_639_1 === searchparam);
  return lang;
};
function LanguageListBox() {
  ...
  const onSelectLanguage = (selectedLanguage) => {
    localStorage.setItem("lang", selectedLanguage.iso_639_1);
    ...
    if (prevQuery) newPathname += "?" + prevQuery;
    nav(newPathname);
  };

  return (
    <Listbox value={selected} onChange={onSelectLanguage}>
      <div className="relative w-full mt-1">
        <Listbox.Button className="relative w-full cursor-default rounded-sm bg-primary shadow-gray-900  text-gray-100 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
          <span className="block truncate">{selected.english_name}</span>
              ...
              <Listbox.Option
                ...
                className={({ active }) =>
                  `relative cursor-default select-none py-2 pl-10 pr-4 ${
                    active ? "bg-secondary text-white" : "text-white bg-black"
                  }`
                }
                value={lang}
              >
